...
module.exports = class ImageGeneratorCtx {
  ...
  async generate(outImgPath) {
    try {
      ...
      this.loadedImage = await this.loadImage(outImgPath);
  ...
  async drawText(args) {
    const bgColor = Jimp.intToRGBA(this.loadedImage.getPixelColor(args.x, args.y));
    const bgColorHex = '#' + rgbHex(bgColor.r, bgColor.g, bgColor.b);
    const color = this.pickTextColorBasedOnBgColorAdvanced(bgColorHex, '#FFFFFF', '#000000');
    await this.page.evaluate((args, color) => {
